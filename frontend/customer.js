...
function carPhotoLayers(c){
  const src=esc(getCarMainPhoto(c));
  return `<img class="car-photo-backdrop" src="${src}" alt="" aria-hidden="true" loading="lazy" decoding="async"><img class="car-photo-main" src="${src}" alt="${esc(c.name)}" loading="lazy" decoding="async">`;
}
...
function footer(){return `<footer class="footer"><div class="wrap"><div class="footer-top"><a class="logo" href="#home"><span class="apex-mark"><svg viewBox="0 0 24 24" width="20" height="20" aria-hidden="true"><path d="M12 2 L22 22 H2 Z" fill="currentColor"/></svg></span> APEX</a><div class="footer-links"><button onclick="guidelines()">Rental info</button><button onclick="terms()">Terms</button><button onclick="contact()">Contact</button></div></div><div class="footer-bottom"><span>© 2026 APEX. All rights reserved.</span><span>Rentals across Pakistan</span><span class="footer-credit">Developed by <strong class="footer-spidy">Spidy</strong> <span class="footer-spider" role="img" aria-label="🕷️ Spider emoji"><svg viewBox="0 0 24 24" width="15" height="15" aria-hidden="true" fill="none"><circle cx="12" cy="6.5" r="2.7" fill="currentColor"/><ellipse cx="12" cy="15" rx="4.2" ry="5.3" fill="currentColor"/><path d="M9.5 8.5 6.5 4 3.5 3M8.8 10.8 4.5 7.5 1.7 9M7.8 13.7 4 14 1.8 17M8 16.2 5.4 20 5 23M14.5 8.5 17.5 4 20.5 3M15.2 10.8 19.5 7.5 22.3 9M16.2 13.7 20 14 22.2 17M16 16.2 18.6 20 19 23" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span></span></div></div></footer>`}
...
function adminFleet(){return `<div class="section-head"><div><h2 style="font-size:23px">Fleet</h2></div><button class="btn" onclick="addFleet()">+ Add vehicle</button></div><div class="cars">${fleet.map(c=>`<div class="car depth-layer"><div class="car-photo">${carPhotoLayers(c)}<span class="pill">${c.status} · ${c.origin}</span></div><div class="car-body"><h3>${esc(c.name)}</h3><p class="small muted" style="margin-top:8px">${c.year} · ${c.brand} · ${money(c.rate)}/day</p><div class="button-row" style="justify-content:flex-start;margin-top:12px"><button class="btn dark" onclick="editFleet(${c.id})">Edit + photos ↗</button><button class="btn ghost" onclick="deleteFleet(${c.id})">Delete</button></div></div></div>`).join('')}</div>`}
...
function card(c){
  ...
  return `<article class="car" data-car-id="${Number(c.id)}"><a class="car-photo" href="#car/${c.id}" aria-label="View ${esc(c.name)}">${carPhotoLayers(c)}<span class="pill car-status ${rented?'is-rented':''}">${esc(status)}</span></a><div class="car-body"><h3><a href="#car/${c.id}">${esc(c.name)}</a></h3><div class="car-rent"><div class="car-daily"><strong>${money(c.rate)}</strong><span>/ day</span></div><small class="car-hourly">${money(v3Hourly(c))} / hour</small></div></div></article>`;
}
function home(){
